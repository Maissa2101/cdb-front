...
export class UpdateComputerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.computerService.getById(this.id).subscribe(
      computer => {
      ...
      error => {
        if (error.status > 499) {
          this.snackBar.open(
            "Can't reach the database. Press F5 or try later.",
            "Close",
            {
  ...
  onSubmit(computer: Computer) {
    ...
    if (this.computerForm.controls.introduced.value) {
      this.computer.introduced = new Date(
        this.computerForm.controls.introduced.value
      )
        .toISOString()
        .substring(0, 10);
    }
    if (this.computerForm.controls.discontinued.value) {
      this.computer.discontinued = new Date(
        this.computerForm.controls.discontinued.value
      )
        .toISOString()
        .substring(0, 10);
    }
    this.computerService.updateComputer(this.computer).subscribe(
      ...
      error => {
        if (error.status === 400) {
          ...
          this.snackBar.open(
            "Can't update the computer. Please try again.",
            "Close",
            {
              panelClass: "snackbar-error",
              duration: 2500
